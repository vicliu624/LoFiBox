...
Audio::Audio(bool internalDAC /* = false */,
  ...
  //    I2S_DAC_CHANNEL_RIGHT_EN = 1,     Enable I2S built-in DAC right channel,
  //    maps to DAC channel 1 on GPIO25 I2S_DAC_CHANNEL_LEFT_EN  = 2,     Enable
  //    I2S built-in DAC left  channel, maps to DAC channel 2 on GPIO26
...
#if ESP_ARDUINO_VERSION_MAJOR >= 2
    m_i2s_config.communication_format =
        (i2s_comm_format_t)(I2S_COMM_FORMAT_STAND_I2S); // Arduino vers. > 2.0.0
#else
    m_i2s_config.communication_format =
...
//---------------------------------------------------------------------------------------------------------------------
#ifndef AUDIO_NO_NETWORK
bool Audio::connecttomarytts(const char *speech, const char *lang,
                             const char *voice) {
...
int Audio::read_ID3_Header(uint8_t *data, size_t len) {
  ...
  // - -
  if (m_controlCounter ==
      5) { // If the frame is larger than 256 bytes, skip the rest
    if (framesize > 256) {
      framesize -= 256;
      headerSize -= 256;
      return 256;
  ...
  if (m_controlCounter == 6) { // Read the value
    ...
    // 0x80). $01 – UCS-2 encoded Unicode with BOM, in ID3v2.2 and ID3v2.3. $02
    // – UTF-16BE encoded Unicode without BOM, in ID3v2.4. $03 – UTF-8 encoded
    // Unicode, in ID3v2.4.
...
int Audio::read_M4A_Header(uint8_t *data, size_t len) {
  ...
  if (m_controlCounter == M4A_TRAK) { // trak
    ...
    if (specialIndexOf(data, "mp4a", len) > 0) {
      ...
      int channel = bigEndian(data + offset + 20,
  ...
  if (m_controlCounter == M4A_AMRDY) { // almost ready
    m_audioDataStart = headerSize;
    //        m_contentlength = headerSize + m_audioDataSize; // after this mdat
    //        atom there may be other atoms
    if (getDatamode() == AUDIO_LOCALFILE) {
#ifndef AUDIO_NO_NETWORK
      AUDIO_INFO("Content-Length: %u", m_contentlength);
#endif
...
int Audio::read_OGG_Header(uint8_t *data, size_t len) {
  ...
  // - -
  if (m_controlCounter ==
      OGG_FIRST) { /* check OGG FIRST PAGES (has no streaming content) */
    uint8_t i = 0;
    ...
    if (specialIndexOf(data + i, "FLAC", 10) == 0) {
      ...
      log_e(
          "ogg/flac support only"); // ogg/vorbis or ogg//opus not supported yet
      stopSong();
      return -1;
    }
    i += 4;
    ...
    i++;
    uint16_t nonah =
        bigEndian(data + i, 2); // number of non audio headers (0x00 = unknown)
    (void)nonah;
    i += 2;
    if (specialIndexOf(data + i, "fLaC", 10) == 0) {
    ...
    if (getChannels() == 2) {
      ...
      while (m_validSamples) {
        if (!m_f_forceMono) { // stereo mode
          ...
          int16_t xy =
              (m_outBuff[m_curSample * 2] + m_outBuff[m_curSample * 2 + 1]) / 2;
          sample[LEFTCHANNEL] = xy;
          sample[RIGHTCHANNEL] = xy;
        }
        playSample(sample);
  ...
  {                                       // local file only
    ...
    case AUDIO_PLAYLISTDATA:
      ...
      if (host) {
        ...
        setDatamode(AUDIO_DATA); // fake datamode, we have no new audiosequence
                                 // yet, so let audio run
      }
      break;
    case AUDIO_DATA:
      if (m_f_ts)
      ...
      if (f_noNewHost) {
        ...
        if (m_f_continue) { // processWebStream() needs more data
          remaintime =
              (int32_t)(m_m3u8_targetDuration * 1000) - (millis() - timestamp1);
  ...
  if (lines) {
    for (int i = 0; i < lines; i++) {
      ...
      if (startsWith(m_playlistContent[i], "#EXT-X-STREAM-INF:")) {
        if (occurence > 0)
          break; // no more than one #EXT-X-STREAM-INF: (can have different
                 // BANDWIDTH)
        occurence++;
        ...
        if (!startsWith(m_playlistContent[i], "http")) {
          ...
          tmp =
              (char *)malloc(strlen(m_lastHost) + strlen(m_playlistContent[i]));
...
void Audio::processLocalFile() {
  ...
  if (m_audioDataSize) {
    availableBytes =
        min(availableBytes, m_audioDataSize + m_audioDataStart - byteCounter);
  }
  ...
  if (!f_stream) {
    if (m_controlCounter != 100) {
      ...
      if ((InBuff.freeSpace() > maxFrameSize) &&
          (m_file_size - byteCounter) > maxFrameSize) {
  ...
  if (m_resumeFilePos) {
    ...
    if (m_avr_bitrate)
      m_audioCurrentTime =
          ((m_resumeFilePos - m_audioDataStart) / m_avr_bitrate) * 8;
...
void Audio::processWebStream() {
  ...
  if (availableBytes) {
    ...
    if (InBuff.bufferFilled() > maxFrameSize &&
        !f_stream) {   // waiting for buffer filled
...
void Audio::processWebFile() {
  ...
  if (m_audioDataSize)
    availableBytes =
        min(m_audioDataSize - (byteCounter - m_audioDataStart), availableBytes);
  ...
  while (true) { // outer while
    ...
    // }
      ...
      latinToUTF8(
          rhl,
          sizeof(
              rhl)); // if already UTF-0 do nothing, otherwise convert to UTF-8
      if (audio_icydescription)
...
void Audio::compute_audioCurrentTime(int bd) {
  ...
  if (!f_CBR) {
    ...
    if (loop_counter == 2) {
      m_avr_bitrate = getBitRate();
      if (m_resumeFilePos) { // if connecttoFS() is called with resumeFilePos !=
                             // 0
        m_audioCurrentTime =
            ((getFilePos() - m_audioDataStart - inBufferFilled()) /
             m_avr_bitrate) *
...
bool Audio::playSample(int16_t sample[2]) {
  ...
  if (getBitsPerSample() ==
      8) { // Upsample from unsigned 8 bits to signed 16 bits
    sample[LEFTCHANNEL] = ((sample[LEFTCHANNEL] & 0xff) - 128) << 8;
    sample[RIGHTCHANNEL] = ((sample[RIGHTCHANNEL] & 0xff) - 128) << 8;
  }

  sample[LEFTCHANNEL] =
      sample[LEFTCHANNEL] >> 1; // half Vin so we can boost up to 6dB in filters
...
//---------------------------------------------------------------------------------------------------------------------
int16_t *
Audio::IIR_filterChain0(int16_t iir_in[2],
                        bool clear) { // Infinite Impulse Response (IIR) filters
...
//---------------------------------------------------------------------------------------------------------------------
int16_t *
Audio::IIR_filterChain1(int16_t iir_in[2],
                        bool clear) { // Infinite Impulse Response (IIR) filters
...
//---------------------------------------------------------------------------------------------------------------------
int16_t *
Audio::IIR_filterChain2(int16_t iir_in[2],
                        bool clear) { // Infinite Impulse Response (IIR) filters
...
uint16_t Audio::readMetadata(uint16_t maxBytes, bool first) {
  ...
  if (metalen < m_chbufSize) {
    uint16_t a =
        _client->readBytes(&m_chbuf[pos_ml], min((uint16_t)(metalen - pos_ml),
                                                 (uint16_t)(maxBytes - 1)));
    res += a;
    pos_ml += a;
