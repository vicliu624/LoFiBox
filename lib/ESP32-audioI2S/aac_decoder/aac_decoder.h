...
typedef struct _AACDecInfo_t {
  /* raw decoded data, before rounding to 16-bit PCM (for postprocessing such as
   * SBR) */
  void *rawSampleBuf[2];
  int rawSampleBytes;
  int rawSampleFBits;
...
typedef struct _ADTSHeader_t {
  ...
  uint8_t home;          /* ignore */
  /* variable */
  uint8_t copyBit; /* 1 bit of the 72-bit copyright ID (transmitted as 1 bit per
                      frame) */
...
typedef struct _ProgConfigElement_t {
  ...
  uint8_t monoMixdown; /* mono mixdown: bit 4 = present flag, bits 3-0 = element
                          number */
  ...
  uint8_t cce[15]; /* [MAX_NUM_BCE] channel coupling elements: bit 4 = switching
                      flag, bits 3-0 = inst tag */
} ProgConfigElement_t;
