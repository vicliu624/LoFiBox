...
public:
  ...
  uint64_t usedBytes() {
    // set SdFatConfig MAINTAIN_FREE_CLUSTER_COUNT non-zero. Then only the first
    // call will take time.
...
private:
  ...
  const uint8_t volumetable[22] = {0,  1,  2,  3,  4,  6,  8,  10,
                                   12, 14, 17, 20, 23, 27, 30, 34,
                                   38, 43, 48, 52, 58, 64}; // 22 elements
  ...
  uint8_t m_expectedPlsFmt =
      FORMAT_NONE; // set in connecttohost (e.g. streaming01.m3u) -> FORMAT_M3U)
  ...
  bool m_f_Log =
      false; // set in platformio.ini  -DAUDIO_LOG and -DCORE_DEBUG_LEVEL=3 or 4
  uint32_t m_stsz_numEntries = 0; // num of entries inside stsz atom (uint32_t)
                                  // - used for M4A local files
  uint32_t m_stsz_position =
      0; // pos of stsz atom within file - used for M4A local files
#ifndef AUDIO_NO_NETWORK
