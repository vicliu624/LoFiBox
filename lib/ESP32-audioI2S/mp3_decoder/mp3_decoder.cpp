...
const int ISFMpeg1[2][7] PROGMEM = {
    {0x00000000, 0x0d8658ba, 0x176cf5d0, 0x20000000, 0x28930a2f, 0x3279a745,
     0x40000000},
    {0x00000000, 0x13207f5c, 0x2120fb83, 0x2d413ccc, 0x39617e16, 0x4761fa3d,
     0x5a827999}};
...
//----------------------------------------------------------------------------------------------------------------------
int CheckPadBit() { return (m_FrameHeader->paddingBit ? 1 : 0); }
//----------------------------------------------------------------------------------------------------------------------
int UnpackFrameHeader(unsigned char *buf) {
  ...
  verIdx = (buf[1] >> 3) & 0x03;
  m_MPEGVersion =
      (MPEGVersion_t)(verIdx == 0 ? MPEG25 : ((verIdx & 0x01) ? MPEG1 : MPEG2));
  m_FrameHeader->layer =
      4 - ((buf[1] >> 1) &
  ...
  if (m_FrameHeader->brIdx) {
    ...
    m_MP3DecInfo->nSlots =
        (int)
            slotTab[m_MPEGVersion][m_FrameHeader->srIdx][m_FrameHeader->brIdx] -
...
int UnpackSideInfo(unsigned char *buf) {
  ...
  for (gr = 0; gr < m_MP3DecInfo->nGrans; gr++) {
    for (ch = 0; ch < m_MP3DecInfo->nChans; ch++) {
      sis =
          &m_SideInfoSub[gr]
                        [ch]; /* side info subblock for this granule, channel */
...
int MP3Decode(unsigned char *inbuf, int *bytesLeft, short *outbuf,
  ...
  *bytesLeft -= (fhBytes + siBytes);
  ...
  for (gr = 0; gr < m_MP3DecInfo->nGrans; gr++) {
    ...
    if (Subband(outbuf + gr * m_MP3DecInfo->nGranSamps * m_MP3DecInfo->nChans) <
        0) {
      MP3ClearBadFrame(outbuf);
...
int DecodeHuffmanPairs(int *xy, int nVals, int tabIdx, int bitsLeft,
  ...
  if (tabType == noBits) {
    ...
    maxBits =
        (int)((((unsigned short)(pgm_read_word(&tBase[0])) >> 0) & 0x000f));
    tBase++;
    padBits = 0;
    while (nVals > 0) {
    ...
    while (nVals > 0) {
      ...
      while (nVals > 0 && cachedBits >= 11) {
        maxBits =
            (int)((((unsigned short)(pgm_read_word(&tCurr[0]))) >> 0) & 0x000f);
...
int MP3Dequantize(int gr) {
  ...
  for (ch = 0; ch < m_MP3DecInfo->nChans; ch++) {
    m_HuffmanInfo->gb[ch] =
        DequantChannel(m_HuffmanInfo->huffDecBuf[ch], m_DequantInfo->workBuf,
...
void PolyphaseStereo(short *pcm, int *vbuf, const uint32_t *coefBase) {
  ...
  rndVal =
      (uint64_t)(1 << ((m_DQ_FRACBITS_OUT - 2 - 2 - 15) - 1 + (32 - m_CSHIFT)));
  ...
  *(pcm + 2 * 16 + 1) = ClipToShort((int)SAR64(sum1R, (32 - m_CSHIFT)),
  ...
  for (i = 15; i > 0; i--) {
    ...
    *(pcm + 1) = ClipToShort((int)SAR64(sum1R, (32 - m_CSHIFT)),
                             m_DQ_FRACBITS_OUT - 2 - 2 - 15);
    *(pcm + 2 * 2 * i + 0) = ClipToShort((int)SAR64(sum2L, (32 - m_CSHIFT)),
                                         m_DQ_FRACBITS_OUT - 2 - 2 - 15);
    *(pcm + 2 * 2 * i + 1) = ClipToShort((int)SAR64(sum2R, (32 - m_CSHIFT)),
                                         m_DQ_FRACBITS_OUT - 2 - 2 - 15);
    pcm += 2;
  }
}
