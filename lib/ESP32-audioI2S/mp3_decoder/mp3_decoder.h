...
static const uint8_t m_MAX_REORDER_SAMPS =
    (192 - 126) * 3; // largest critical band for short blocks (see sfBandTable)
...
typedef enum { /* map these to the corresponding 2-bit values in the frame
                  header */
...
typedef struct HuffmanInfo {
  ...
  int nonZeroBound[m_MAX_NCHAN]; /* number of coeffs in huffDecBuf[ch] which can
                                    be > 0 */
...
typedef struct IMDCTInfo {
  ...
  int overBuf[m_MAX_NCHAN][m_MAX_NSAMP / 2]; /* overlap-add buffer (by symmetry,
                                                only need 1/2 size) */
...
typedef struct ScaleFactorInfoSub { /* max bits in scalefactors = 5, so use
                                       char's to save space */
  char l[23];                       /* [band] */
  char s[13][3];                    /* [band][window] */
} ScaleFactorInfoSub_t;

typedef struct ScaleFactorJS { /* used in MPEG 2, 2.5 intensity (joint) stereo
                                  only */
  int intensityScale;
  int slen[4];
  int nr[4];
...
const int c6[3] = {0x7ba3751d, 0x5a82799a,
...
const int pow14[4] PROGMEM = {0x7fffffff, 0x6ba27e65, 0x5a82799a, 0x4c1bf829};
 ...
 */
const unsigned int poly43lo[5] PROGMEM = {0x29a0bda9, 0xb02e4828, 0x5957aa1b,
                                          0x236c498d, 0xff581859};
const unsigned int poly43hi[5] PROGMEM = {0x10852163, 0xd333f6a4, 0x46e9408b,
                                          0x27c2cef0, 0xfef577b4};

/* pow(2, i*4/3) as exp and frac */
const int pow2exp[8] PROGMEM = {14, 13, 11, 10, 9, 7, 6, 5};

const int pow2frac[8] PROGMEM = {0x6597fa94, 0x50a28be6, 0x7fffffff,
...
int DecodeHuffman(unsigned char *buf, int *bitOffset, int huffBlockBits, int gr,
                  int ch);
