...
bool FLACDecoder_AllocateBuffers(void) {
  if (psramFound()) {
    ...
    if (!FLACMetadataBlock) {
      FLACMetadataBlock =
          (FLACMetadataBlock_t *)ps_malloc(sizeof(FLACMetadataBlock_t));
    }
    if (!FLACsubFramesBuff) {
      FLACsubFramesBuff =
          (FLACsubFramesBuff_t *)ps_malloc(sizeof(FLACsubFramesBuff_t));
    }
  } else {
    if (!FLACFrameHeader) {
      FLACFrameHeader = (FLACFrameHeader_t *)malloc(sizeof(FLACFrameHeader_t));
    }
    if (!FLACMetadataBlock) {
      FLACMetadataBlock =
...
void alignToByte() { m_bitBufferLen -= m_bitBufferLen % 8; }
//----------------------------------------------------------------------------------------------------------------------
...
int8_t FLACDecode(uint8_t *inbuf, int *bytesLeft, short *outbuf) {
  ...
  if (m_status == OUT_SAMPLES) { // Write the decoded samples
    ...
    // therefore we need often more than one loop (split outputblock into
    // pieces)
    uint16_t blockSize;
  ...
  //    FLACMetadataBlock->numChannels * (16/8); log_i("m_compressionRatio % f",
  //    m_compressionRatio);
  *bytesLeft = m_bytesAvail;
  m_status = DECODE_FRAME;
  return ERR_FLAC_NONE;
...
//----------------------------------------------------------------------------------------------------------------------
uint8_t FLACGetBitsPerSample() { return FLACMetadataBlock->bitsPerSample; }
//----------------------------------------------------------------------------------------------------------------------
uint8_t FLACGetChannels() { return FLACMetadataBlock->numChannels; }
//----------------------------------------------------------------------------------------------------------------------
...
uint32_t FLACGetAudioFileDuration() {
  if (FLACGetSampRate()) {
    uint32_t afd =
        FLACGetTotoalSamplesInStream() / FLACGetSampRate(); // AudioFileDuration
    return afd;
  }
  return 0;
}
...
int8_t decodeSubframes() {
  ...
  } else if (8 <= FLACFrameHeader->chanAsgn &&
             FLACFrameHeader->chanAsgn <= 10) {
