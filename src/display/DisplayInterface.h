...
public:
  LilyGo_Display(DriverBusType type, bool full_refresh)
      : _offset_x(0), _offset_y(0), _rotation(0), _interface(type),
        _full_refresh(full_refresh) {}
  virtual void setRotation(uint8_t rotation) = 0;
  virtual uint8_t getRotation() = 0;
  virtual void pushColors(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                          uint16_t *color) = 0;
  ...
  virtual uint8_t getPoint(int16_t *x, int16_t *y, uint8_t get_point) {
    return 0;
  }
...
public:
  ...
  LilyGoDispArduinoSPI(uint16_t width, uint16_t height,
                       const CommandTable_t *init_list, size_t init_list_length,
                       const DispRotationConfig_t *rotation_config)
      : _init_width(width), _init_height(height), _init_list(init_list),
        _init_list_length(init_list_length),
  ...
  uint8_t getRotation();
  void pushColors(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                  uint16_t *color);
