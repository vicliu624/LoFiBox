...
int build_playlist_tracks(UiScreen& screen)
{
    ...
    if (screen.state.current_playlist == 0) {
        for (int i = 0; i < screen.on_the_go_count && screen.playlist_count < app::kMaxPlaylistTracks; ++i) {
            screen.playlist_tracks[screen.playlist_count++] = screen.on_the_go[i];
        }
        return screen.playlist_count;
    }

    for (int i = 0; i < screen.library->track_count && screen.playlist_count < app::kMaxPlaylistTracks; ++i) {
...
void populate(UiScreen& screen)
{
    ...
    if (build_playlist_tracks(screen) == 0) {
        components::add_item(screen, "Empty", nullptr, UiIntentKind::None, PageId::None);
        return;
    }
    ...
    for (int i = 0; i < screen.playlist_count; ++i) {
        ...
        components::add_item(screen, track.title, track.artist, UiIntentKind::PlayTrack, PageId::NowPlaying, idx);
    }
}
