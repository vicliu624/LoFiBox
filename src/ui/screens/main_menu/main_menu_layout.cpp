...
MenuLayout create_menu(lv_obj_t *content) {
  ...
  lv_coord_t label_top =
      dot_top - label_gap - label_height + scale_y(kBaseLabelYOffset, height);
  ...
  lv_coord_t row_width =
      icon_size * kVisibleItems + icon_gap * (kVisibleItems - 1);
  lv_coord_t row_left =
      (width - row_width) / 2 + scale_x(kBaseRowOffset, width);
  ...
  for (int i = 0; i < kVisibleItems; ++i) {
    ...
    lv_obj_align(item.button, LV_ALIGN_TOP_LEFT,
                 row_left + i * (icon_size + icon_gap), icon_top);
  ...
  lv_coord_t arrow_y =
      icon_top + (icon_size / 2) - (arrow_h / 2) + kBaseArrowOffsetY;
  ...
  lv_obj_set_pos(refs.arrow_right,
                 width - scale_x(kBaseArrowInset, width) - arrow_w, arrow_y);
  ...
  for (int i = 0; i < kDots; ++i) {
    ...
    lv_obj_align(refs.dots[i], LV_ALIGN_TOP_LEFT,
                 dots_left + i * (dot_size + dot_gap), dots_top);
  ...
  if (item.icon) {
    if (icon) {
      ...
      if (icon->header.w > 0) {
        ...
        uint32_t zoom = (static_cast<uint32_t>(target) * 256U) /
