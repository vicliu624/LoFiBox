...
const char* kLeftLabels[kVisibleBands] = {"Bass", "200 Hz", "400 Hz", "1 kHz", "2.5 kHz", "Treble"};
const char* kBottomLabels[kVisibleBands] = {"Bass", "200", "400", "1k", "2.5k", "Treble"};
...
void build(UiScreen& screen)
{
    ...
    for (int i = 0; i < kVisibleBands; ++i) {
        ...
        if (screen.view.eq.sliders[i]) {
            ...
            lv_slider_set_value(screen.view.eq.sliders[i], app::eq::get_band(band), LV_ANIM_OFF);
        }
        if (screen.view.eq.labels[i]) {
            ...
            if (i == 0) {
                lv_obj_set_style_text_color(screen.view.eq.labels[i], lv_color_hex(0xffffff), LV_PART_MAIN);
            } else {
                lv_obj_set_style_text_color(screen.view.eq.labels[i], lv_color_hex(0xd8dadd), LV_PART_MAIN);
            }
            lv_obj_set_style_text_align(screen.view.eq.labels[i], LV_TEXT_ALIGN_LEFT, LV_PART_MAIN);
        ...
        if (screen.view.eq.labels[i + 6]) {
            ...
            lv_obj_set_style_text_color(screen.view.eq.labels[i + 6], lv_color_hex(0x9ba1a8), LV_PART_MAIN);
            lv_obj_set_style_text_align(screen.view.eq.labels[i + 6], LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
        }
    }
