...
constexpr uint32_t kBacklightTimeoutsMs[] = {10000,  30000,  60000,
                                             120000, 300000, 0};
constexpr uint32_t kSleepTimeoutsMs[] = {120000, 300000, 600000, 0};

uint32_t cycle_timeout(uint32_t current, const uint32_t *options,
                       size_t count) {
  if (!options || count == 0) {
    return current;
  }
...
const char *current_page_title(const UiScreen &screen) {
  if (screen.state.current == PageId::PlaylistDetail &&
      screen.state.current_playlist >= 0 &&
      screen.state.current_playlist <
          static_cast<int>(sizeof(kPlaylistNames) /
                           sizeof(kPlaylistNames[0]))) {
    return kPlaylistNames[screen.state.current_playlist];
  }
  if (screen.state.current == PageId::Albums &&
...
void update_battery(UiScreen &screen) {
  ...
  char buf[16] = {0};
  ui_format_battery(board.getBatteryLevel(), board.isCharging(), buf,
                    sizeof(buf));
  lv_label_set_text(screen.view.root.top_battery, buf);
  if (screen.view.root.top_signal) {
    char vol_buf[16] = {0};
    if (screen.player) {
      snprintf(vol_buf, sizeof(vol_buf), "VOL %u",
               static_cast<unsigned>(app::player_get_volume(*screen.player)));
    } else {
      snprintf(vol_buf, sizeof(vol_buf), "VOL --");
    }
    lv_label_set_text(screen.view.root.top_signal, vol_buf);
  }
}
...
void play_track(UiScreen &screen, int track_index) {
  ...
  Serial.printf(
      "[UI] play_track idx=%d title=\"%s\" artist=\"%s\" album=\"%s\"\n",
      track_index, track.title ? track.title : "",
...
void reset_items(UiScreen &screen) { screen.items_count = 0; }

ListItem *add_item(UiScreen &screen, const char *left, const char *right,
                   UiIntentKind action, PageId next, int value, int value2,
...
void update_topbar(UiScreen &screen) {
  ...
  if (screen.state.current == PageId::MainMenu) {
    ...
    if (screen.view.root.top_title_fade_right) {
      lv_obj_add_flag(screen.view.root.top_title_fade_right,
                      LV_OBJ_FLAG_HIDDEN);
    }
    update_battery(screen);
    return;
  }
  if (screen.state.current == PageId::NowPlaying && screen.player &&
      screen.library && screen.player->current_index >= 0 &&
    ...
    String safe_title = text::single_line(
        (track.title && track.title[0]) ? track.title : "Now Playing");
    ...
    lv_label_set_long_mode(screen.view.root.top_title,
                           LV_LABEL_LONG_SCROLL_CIRCULAR);
    lv_obj_set_style_text_align(screen.view.root.top_left, LV_TEXT_ALIGN_LEFT,
                                LV_PART_MAIN);
    lv_obj_set_style_text_align(screen.view.root.top_title,
                                LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
    lv_obj_set_style_text_line_space(screen.view.root.top_left, 0,
                                     LV_PART_MAIN);
    lv_obj_set_style_text_line_space(screen.view.root.top_title, 0,
                                     LV_PART_MAIN);
    ...
    const lv_font_t *font_left =
        lv_obj_get_style_text_font(screen.view.root.top_left, LV_PART_MAIN);
    if (font_left) {
    ...
    const lv_font_t *font_title =
        lv_obj_get_style_text_font(screen.view.root.top_title, LV_PART_MAIN);
    if (font_title) {
    ...
    lv_obj_set_style_text_color(screen.view.root.top_left,
                                lv_color_hex(0xf2f2f2), LV_PART_MAIN);
    lv_obj_set_style_text_color(screen.view.root.top_title,
                                lv_color_hex(0x8f949a), LV_PART_MAIN);

    if (screen.view.root.top_title_fade_left &&
        screen.view.root.top_title_fade_right) {
      bool show_fade = true;
      const lv_font_t *font =
          lv_obj_get_style_text_font(screen.view.root.top_title, LV_PART_MAIN);
      if (font) {
        lv_point_t text_size{};
        lv_coord_t letter_space = lv_obj_get_style_text_letter_space(
            screen.view.root.top_title, LV_PART_MAIN);
        lv_coord_t line_space = lv_obj_get_style_text_line_space(
            screen.view.root.top_title, LV_PART_MAIN);
        lv_txt_get_size(&text_size, center.c_str(), font, letter_space,
                        line_space, LV_COORD_MAX, LV_TEXT_FLAG_NONE);
      ...
      if (show_fade) {
        lv_obj_clear_flag(screen.view.root.top_title_fade_left,
                          LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(screen.view.root.top_title_fade_right,
                          LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_size(screen.view.root.top_title_fade_left, fade_w, bar_h);
        lv_obj_align(screen.view.root.top_title_fade_left, LV_ALIGN_LEFT_MID, 0,
                     0);
        lv_obj_set_size(screen.view.root.top_title_fade_right, fade_w, bar_h);
        lv_obj_align(screen.view.root.top_title_fade_right, LV_ALIGN_RIGHT_MID,
                     0, 0);
        lv_obj_move_foreground(screen.view.root.top_title_fade_left);
        lv_obj_move_foreground(screen.view.root.top_title_fade_right);
      } else {
        lv_obj_add_flag(screen.view.root.top_title_fade_left,
                        LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(screen.view.root.top_title_fade_right,
                        LV_OBJ_FLAG_HIDDEN);
      }
...
void update_main_menu(UiScreen &screen) {
  ...
  for (int i = 0; i < screens::main_menu::layout::kVisibleItems; ++i) {
    ...
    screens::main_menu::layout::set_item(menu.items[i],
                                         screen.items[item_idx].left.c_str(),
    ...
    if (menu.items[i].icon) {
      ...
      if (icon && icon->header.w > 0) {
        uint32_t zoom = (static_cast<uint32_t>(icon_w) * 256U) /
                        static_cast<uint32_t>(icon->header.w);
        if (zoom == 0) {
          zoom = 256;
        }
        lv_img_set_zoom(menu.items[i].icon, zoom);
      }
      lv_obj_set_style_img_opa(menu.items[i].icon,
                               (scale >= 0.99f) ? LV_OPA_COVER : LV_OPA_70,
                               LV_PART_MAIN);
    }
    if (menu.items[i].label) {
      ...
      lv_obj_set_style_text_opa(menu.items[i].label,
                                (scale >= 0.99f) ? LV_OPA_COVER : LV_OPA_70,
                                LV_PART_MAIN);
    }
  ...
  int active_dot =
      (total > 0)
          ? (screen.state.menu_index % screens::main_menu::layout::kDots)
          : 0;
  for (int i = 0; i < screens::main_menu::layout::kDots; ++i) {
    if (!menu.dots[i]) {
      continue;
...
NavCommand handle_intent(UiScreen &screen, const UiIntent &intent) {
  ...
  case UiIntentKind::ToggleRepeat:
    ...
    cmd.type = NavCommand::Type::Rebuild;
    break;
  case UiIntentKind::CycleBacklightTimeout: {
    uint32_t next = cycle_timeout(
        lvHelperGetBacklightTimeoutMs(), kBacklightTimeoutsMs,
        sizeof(kBacklightTimeoutsMs) / sizeof(kBacklightTimeoutsMs[0]));
    lvHelperSetBacklightTimeoutMs(next);
    cmd.type = NavCommand::Type::Rebuild;
    break;
  }
  case UiIntentKind::CycleSleepTimeout: {
    uint32_t next =
        cycle_timeout(lvHelperGetSleepTimeoutMs(), kSleepTimeoutsMs,
                      sizeof(kSleepTimeoutsMs) / sizeof(kSleepTimeoutsMs[0]));
    lvHelperSetSleepTimeoutMs(next);
    cmd.type = NavCommand::Type::Rebuild;
    break;
  }
...
void build_page(UiScreen &screen) {
  ...
  if (screen.view.root.top_title_wrap) {
    lv_obj_set_style_bg_opa(screen.view.root.top_title_wrap, LV_OPA_0,
                            LV_PART_MAIN);
    lv_obj_set_style_border_width(screen.view.root.top_title_wrap, 0,
                                  LV_PART_MAIN);
