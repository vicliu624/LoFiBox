...
// This class, LilyGoKeyboard, inherits from Adafruit_TCA8418 and is designed to
...
public:
  ...
  // It takes an integer representing the key state and a reference to a
  // character to store the key value.
  ...
  // Typedef for a callback function called when adjusting the keyboard
  // backlight.
  using BacklightCallback = void (*)(uint8_t level);

  // Typedef for a callback function called when keyboard press or release raw
  // code.
   ...
   */
  bool begin(const LilyGoKeyboardConfigure_t &config, TwoWire &w, uint8_t irq,
   ...
   * @note  When this callback is set, the program only returns the original key
...
private:
   ...
   * @param pressed true if the key is pressed, false if released.
   * @param c A pointer to a character to store the result of the special key
   * action.
