...
bool init_aw88298() {
  ...
  auto writeAw = [](uint8_t reg, uint16_t value) {
    uint8_t payload[2] = {static_cast<uint8_t>(value >> 8),
...
void CardputerBoard::setBrightness(uint8_t level) { brightness_ = level; }

uint8_t CardputerBoard::getBrightness() const { return brightness_; }

bool CardputerBoard::hasKeyboard() const { return keyboard_ready_; }
...
bool CardputerBoard::isRTCReady() const { return false; }

bool CardputerBoard::isCharging() const { return false; }

int CardputerBoard::getBatteryLevel() const { return -1; }
...
void CardputerBoard::displayPushColors(uint16_t x, uint16_t y, uint16_t w,
                                       uint16_t h, const uint16_t *colors) {
  M5Cardputer.Display.pushImage(x, y, w, h, colors);
}

bool CardputerBoard::isSDReady() const { return sd_ready_; }

bool CardputerBoard::initAudio(uint8_t &bclk, uint8_t &lrck, uint8_t &dout,
                               int8_t &mclk) {
  (void)mclk;
