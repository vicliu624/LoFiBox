...
class BoardBase
{
  public:
    ...
    virtual void displayPushColors(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* colors) = 0;

    // Storage
    virtual bool isSDReady() const = 0;

    // Audio
    virtual bool initAudio(uint8_t& bclk, uint8_t& lrck, uint8_t& dout, int8_t& mclk) = 0;
};
