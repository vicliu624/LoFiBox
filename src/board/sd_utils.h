...
inline bool
installSpiSd(Lockable &bus, int sd_cs, uint32_t spi_hz, const char *mount_point,
             const int *extra_cs, size_t extra_cs_count,
             uint8_t *out_card_type = nullptr,
             uint32_t *out_card_size_mb = nullptr, bool use_lock = true) {
  if (sd_cs < 0) {
    return false;
  }
  ...
  // Use the same SPI host as the rest of the board to avoid dual-host pin
  // conflicts.
  SPI.end();
  delay(2);
  ...
  Serial.printf("[SD] SPI pins sck=%d miso=%d mosi=%d cs=%d hz=%lu\n", SCK,
                MISO, MOSI, sd_cs, (unsigned long)spi_hz);
  for (size_t i = 0; i < extra_cs_count; ++i) {
    Serial.printf("[SD] extra CS pin=%d level=%d\n", extra_cs[i],
                  digitalRead(extra_cs[i]));
  }
  ...
  if (locked) {
    // Try a small frequency fallback ladder; some SD cards/rails are picky at
    // boot.
