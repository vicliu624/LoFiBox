...
static void keyboard_isr() { keyboard_interrupted = true; }

LilyGoKeyboard::LilyGoKeyboard()
    : _backlight(-1), _brightness(0), _irq(0), cb(nullptr),
      repeat_function(false), symbol_key_pressed(false), cap_key_pressed(false),
      alt_key_pressed(false), lastState(false), lastKeyVal('\0'),
      lastPressedTime(0) {}

LilyGoKeyboard::~LilyGoKeyboard() {}

void LilyGoKeyboard::setPins(int backlight) { _backlight = backlight; }
...
uint8_t LilyGoKeyboard::getBrightness() { return _brightness; }

bool LilyGoKeyboard::begin(const LilyGoKeyboardConfigure_t &config, TwoWire &w,
                           uint8_t irq, uint8_t sda, uint8_t scl) {
  if (_backlight != -1) {
...
#else
    ledcSetup(LEDC_BACKLIGHT_CHANNEL, LEDC_BACKLIGHT_FREQ,
              LEDC_BACKLIGHT_BIT_WIDTH);
  ...
  // Configure the matrix size (using the number of rows and columns currently
  // mapped)
...
void LilyGoKeyboard::setCallback(KeyboardReadCallback cb) { this->cb = cb; }
...
void LilyGoKeyboard::setRepeat(bool enable) { repeat_function = enable; }
...
int LilyGoKeyboard::getKey(char *c) {
  ...
  if (millis() - interval > 100) {
    // Polling detects whether there is an ignored state in the interrupt status
    // that has not been processed. The polling speed affects the response speed
    // of the keyboard.
    interval = millis();
...
int LilyGoKeyboard::handleSpecialKeys(uint8_t k, bool pressed, char *c) {
  ...
  KEYBOARD_LOG("[Keyboard] special key=%u pressed=%d\n",
               static_cast<unsigned int>(k), pressed ? 1 : 0);
...
char LilyGoKeyboard::handleSpaceAndNullChar(char keyVal, char &lastKeyVal,
                                            bool &pressed) {
...
void LilyGoKeyboard::printDebugInfo(bool pressed, uint8_t k, char keyVal) {
  KEYBOARD_LOG("Debug: symbol=%d, caps=%d, alt=%d\n", symbol_key_pressed,
               cap_key_pressed, alt_key_pressed);
...
int LilyGoKeyboard::update(char *c) {
  ...
  // value and does not continue with subsequent processing. The user needs to
  ...
  if (row >= _config->kb_rows) {
    log_e("Key values out of range are ignored,current  row:%d k:%d , "
          "_config->kb_cols:%d\n",
